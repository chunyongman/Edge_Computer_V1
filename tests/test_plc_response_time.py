...
def test_plc_response_time():
    ...
    print("="*70)
    print(f"총 측정 횟수: 50회")
    print(f"성공: {success_count}회, 실패: {failed_count}회\n")

    print(f"[1. 응답 시간 통계] (AI 계산 완료 → PLC 통신 완료)")
    ...
    print(f"  PLC 통신 평균: {avg_plc_time*1000:.0f}ms\n")
    ...
    # 합격 기준 (목표: 1.0초 이내)
    criterion1_pass = avg_response <= 1.0  # 평균 1.0초 이하
    criterion2_pass = max_response < 1.5   # 최대 1.5초 미만 (여유)
    criterion3_pass = success_count >= 45  # 성공률 90% 이상

    print(f"  기준 1 - 평균 시간 (≤1.0초): {avg_response:.3f}초 {'[PASS]' if criterion1_pass else '[FAIL]'}")
    print(f"  기준 2 - 최대 시간 (<1.5초): {max_response:.3f}초 {'[PASS]' if criterion2_pass else '[FAIL]'}")
    print(f"  기준 3 - 통신 성공률 (≥90%): {success_count}/50 ({success_count*2}%) {'[PASS]' if criterion3_pass else '[FAIL]'}\n")
    ...
    if final_pass:
        ...
        if not criterion1_pass:
            print(f"  - 평균 응답시간 초과: {avg_response:.3f}초 > 1.0초")
        if not criterion2_pass:
            print(f"  - 최대 응답시간 초과: {max_response:.3f}초 >= 1.5초")
        if not criterion3_pass:
            print(f"  - 통신 성공률 미달: {success_count}/50 < 45/50")
    print("="*70 + "\n")
    ...
    summary_df = pd.DataFrame({
        '항목': ['평균 응답시간', '최소 응답시간', '최대 응답시간', '표준편차', 'AI 추론 평균', 'PLC 통신 평균', '통신 성공률', '최종 판정'],
        '값': [
            f'{avg_response:.3f}초',
            f'{min_response:.3f}초',
            f'{max_response:.3f}초',
            ...
            f'{success_count*2}% ({success_count}/50)',
